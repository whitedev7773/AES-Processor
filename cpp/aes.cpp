...
void AddRoundKey(state_t& s, const byte* rk) {
  for (int c = 0; c < 4; c++)
    for (int r = 0; r < 4; r++)
      s[r][c] ^= rk[c*4 + r];
}
...
void KeyExpansion(const byte* key, byte* rk) {
  memcpy(rk, key, 16);
  for (int i = 4; i < 44; i++) {
    word temp = ((word*)rk)[i-1];
    if (i % 4 == 0) {
      temp = (temp<<8) | (temp>>24);
      byte* t = (byte*)&temp;
    ...
    ((word*)rk)[i] = ((word*)rk)[i-4] ^ temp;
  }
}


// ===================== AES TOP =====================
void AES_Encrypt(byte* in, byte* out, byte* rk) {
  state_t s;
  memcpy(s, in, 16);
...
void AES_Decrypt(byte* in, byte* out, byte* rk) {
  state_t s;
  memcpy(s, in, 16);
...
void print_block(const char* tag, byte* b) {
  std::cout << tag << ": ";
  for(int i=0;i<16;i++)
...
int main() {
  ...
  AES_Encrypt(pt, ct, roundKeys);
  AES_Decrypt(ct, dec, roundKeys);

  print_block("PLAINTEXT             ", pt);
  print_block("ENCRYPTED (CIPHERTEXT)", ct);
  print_block("DECRYPTED             ", dec);
}
